...
<div class="col-lg-12" style="line-height:30px;">
  <form method="GET" action="<?php echo URL::to('admin/compostreport'); ?>" class ="form-horizontal" role ="form">
...
<table id="table_dashboard" class='table table-striped table-bordered datatables-simple'>
  <thead>
      <tr>
        <th>Sr. No.</th>
        <th>Compost Date</th>
  ...
  <tbody>
      ...
      <tr>
        <td>{{ ($k + 1) }}</td>
        <td>{{ $row->CompostDate }}</td>
        <td>{{ $row->CompostQty }}</td>
        <td>{{ $row->SalesQty }}</td>
      ...
      <tfoot>
        ...
        <td>{{ $result->sum('CompostQty')  }}</td>
        <td>{{ $result->sum('SalesQty') }}</td>
        <td>{{ $result->sum('SalesAmount') }}</td>
      </tr>
      </tfoot>
  </tbody>
</table>
